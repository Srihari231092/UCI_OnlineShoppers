...
def expand_date_cols(row, colname):
    weekDays = ("Monday","Tuesday","Wednesday","Thursday","Friday", "Saturday", "Sunday")
    qnum = [1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4]
    v = row[colname]
    row[colname+"_year"] = v.year
    row[colname+"_month"] = v.month
    row[colname+"_date"] = v.day
    row[colname+"_dayofweek"] = weekDays[int(v.weekday())]
    row[colname+"_weekofyear"] = v.isocalendar()[1]
    row[colname+"_quarter"] = qnum[v.month-1]
    return row
